...
def extract_spectrum_data(exp):
    """Extracts Drift Time (DT), Retention Time (RT), and m/z for the first 1000 spectra."""
    drift_times = []
    retention_times = []
    mz_values = []

    for i, spectrum in enumerate(exp.getSpectra()[:100000]):  # First 1000 spectra
        drift_time = spectrum.getDriftTime()
        rt = spectrum.getRT()  # Retention time extraction
        mz = spectrum.getPrecursors()[0].getMZ() if spectrum.getPrecursors() else None

        # Debugging statements
        print(f"[DEBUG] Spectrum {i + 1}: DT={drift_time}, RT={rt}, m/z={mz}")
    ...
    # Display the first 1000 rows in the terminal
    print("\nFirst 1000 spectra (Drift Time, Retention Time & m/z):")
    print(df.to_string(index=False))  # Display DataFrame in a clean format
