...
def flag_duplicates(adjusted_df, mass_error_ppm=10, ccs_tolerance=2.0):
    df = adjusted_df.copy()
    ...
    # Compare each row with every other row (upper triangle only)
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            # Check if both m/z and CCS are within tolerances
            if abs(mz[i] - mz[j]) <= max(mass_tol_i, mass_tol_j) and abs(
                ccs[i] - ccs[j]
            ) <= max(ccs_tol_i, ccs_tol_j):
