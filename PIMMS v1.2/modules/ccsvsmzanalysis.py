...
def load_and_combine_data(data_filepath, library_filepath):
    """
    ...
    try:
        ...
        adjusted_df["join_mz"] = adjusted_df["m/z"]
        library_df["join_mz"] = library_df["m/z"]
...
if __name__ == "__main__":
    # 1. Create a sample DataFrame with all required columns

    file_path = r"PIMMS v1.2\data\250918_SealsPIMMS.csv"
    library_file_path = r"PIMMS v1.2\import folder\test_library.csv"

    adjusted_df = load_and_combine_data(file_path, library_file_path)
    print(adjusted_df.head())
    # 2. Run the first analysis step to find homologous series
    mass_groups_df = find_homologous_series(
        adjusted_df, selected_repeating_units={"CF2": 49.9968064}
    )

    print(mass_groups_df.head())
