...
df_scaled = df.copy()
df_scaled[["PrecursorMz", "PrecursorCCS", "log_mz"]] = scaler.fit_transform(
    df[["PrecursorMz", "PrecursorCCS", "log_mz"]]
)

# Define models for scaled data
scaled_models = {
    "Linear (scaled)": dmatrix("PrecursorMz", df_scaled, return_type="dataframe"),
    "Logarithmic (scaled)": dmatrix("log_mz", df_scaled, return_type="dataframe"),
    "Spline (scaled)": dmatrix(
        "bs(log_mz, df=5, include_intercept=False)", df_scaled, return_type="dataframe"
    ),
}
...
# Run KFold cross-validation for each model (both scaled and original)
for model_name, X in {**models, **scaled_models}.items():
