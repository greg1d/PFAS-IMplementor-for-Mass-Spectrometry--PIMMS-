...
def search_feature(
    ...
    for file_path in excel_files:
        for sheet_option in sheet_options:
            ...
            try:
                ...
                demp_column = next(
                    (col for col in df.columns if col.endswith(".d.DeMP")), None
                )

                # Ensure the necessary columns exist
                if (
                    ccs_column not in df.columns
                    or retention_time_column not in df.columns
                    or demp_column is None
                    or dt_column not in df.columns
                ):
                    continue
    ...
    if results:
        ...
        save_option = messagebox.askyesno(
            "Save Results", "Do you want to save the results to an Excel file?"
        )
        if save_option:
...
def visualize_ccs_vs_mz(result_df):
    ...
    ax.set_title(
        f"CCS vs m/z {title}",
        fontdict={"fontsize": 10, "fontweight": "bold", "fontname": "Arial"},
    )
    ...
    fig.canvas.mpl_connect(
        "key_press_event",
        lambda event: reset_zoom(event, ax, original_xlim, original_ylim),
    )
...
def visualize_ccs_vs_rt(result_df):
    ...
    global original_xlim, original_ylim
    original_xlim = (
        result_df["Retention Time"].min() - 0.2,
        result_df["Retention Time"].max() + 0.2,
    )
    original_ylim = (result_df["CCS"].min() - 3, result_df["CCS"].max() + 3)
    ax.set_xlim(original_xlim)
    ax.set_ylim(original_ylim)
...
def search_button_click():
    ...
    result_df = search_feature(
        name_or_class,
        mz_value,
        ppm_error=ppm_tolerance,
...
tk.Checkbutton(window, text="Likely", variable=likely_var).grid(
    row=3, column=0, padx=10, pady=10
)
tk.Checkbutton(window, text="Tentative (EPA Match)", variable=epa_var).grid(
    row=3, column=1, padx=10, pady=10
)
tk.Checkbutton(window, text="Tentative (No EPA Match)", variable=no_epa_var).grid(
...
ccs_vs_mz_button = tk.Button(
    window,
    text="Visualize CCS vs m/z",
    state=tk.DISABLED,
    command=lambda: visualize_ccs_vs_mz(ccs_vs_mz_button.result_df),
)
ccs_vs_mz_button.grid(row=6, column=0, padx=10, pady=10)

ccs_vs_rt_button = tk.Button(
    window,
    text="Visualize CCS vs Retention Time",
    state=tk.DISABLED,
    command=lambda: visualize_ccs_vs_rt(ccs_vs_rt_button.result_df),
)
