...
directory_path = r"D:\2023 gators\PIMMS data\PIMMS Processing\Processed files only"

# Find all Excel files in the directory that end with "PIMMS_processed.xlsx"
...
for file_path in excel_files:
    ...
    try:
        ...
        if (
            ...
            and "CCS" in df.columns
            and "Retention Time" in df.columns
        ):
            df.dropna(subset=["Name_or_Class"]).apply(
                lambda row: process_chemicals(
                    row["Name_or_Class"],
                    row["CCS"],
                    row["Retention Time"],
                    file_name,
                    unique_chemicals_in_file,
                ),
                axis=1,
            )
        else:
            print(
                f"Required columns ('Name_or_Class', 'CCS', 'Retention Time') not found in {file_path}"
            )
...
chemical_counts_df = pd.DataFrame(
    list(chemical_counts.items()), columns=["Chemical", "Count"]
)
...
output_path = os.path.join(
    directory_path, "Likely chemical_detection_summary_with_ccs_rt_and_frequency.xlsx"
)
...
for col in ccs_cols + rt_cols:
    ...
    for row_idx, z_score in enumerate(
        z_score_col, start=2
    ):  # Start at 2 because Excel rows are 1-indexed
        if pd.notna(z_score) and abs(z_score) > 2:
            cell = ws.cell(
                row=row_idx, column=final_df.columns.get_loc(col) + 1
...
print(
    f"Chemical detection summary with CCS, Retention Time, and detection frequency saved to {output_path}"
)
