...
epa_library_path = r'F:\Twins Project (2.24-)\Non-target work\Import Folder\Kauffman M_H Library.xlsx'
epa_library = pd.read_excel(epa_library_path)
library_masses = epa_library['MONOISOTOPIC MASS'].values
library_preferred_names = epa_library.set_index('MONOISOTOPIC MASS')['PREFERRED NAME'].to_dict()

# Load the neutral loss masses from the specified Excel file
neutral_loss_path = r'F:\Twins Project (2.24-)\Non-target work\Import Folder\Neutral loss masses.xlsx'
...
reference_peaks_df = pd.read_excel(reference_excel_path, sheet_name='MPFAC-HIF-ES Peaks')
...
error_peaks_df = pd.read_excel(reference_excel_path, sheet_name='MPFAC-HIF-ES Error Peaks')
...
def match_external_library(mz_value, library_masses, library_preferred_names, ppm_threshold=10):
    matches = []
...
for file_path in csv_files:
    ...
    header_name = file_name.split('.d')[0].split()[-1]  # Extract the number for the header

    # Define the order of scores from best to worst
    score_order = ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'D-', 'E']

    # Filter rows based on the score being greater than 'E'
    filtered_df = df[df['Score'].apply(lambda x: score_order.index(x) <= score_order.index('E'))]
    ...
    def eliminate_close_rows(df):
        df = df.sort_values(by='m/z').reset_index(drop=True)  # Sort by m/z and reset index
        ...
        for i in range(len(df)):
            ...
            potential_matches = df[(df['m/z'] > df.loc[i, 'm/z'] - 0.01) & (df['m/z'] < df.loc[i, 'm/z'] + 0.01)]
    ...
    def eliminate_mass_shift_rows(df):
        ...
        for i in range(len(df)):
            ...
            ccs1 = df.loc[i, 'CCS']
            intensity1 = df.loc[i, intensity_column]
            retention_time1 = df.loc[i, 'Retention Time']  # Get retention time of the first peak
        ...
        # Get potential matches to reduce calculations - this identifies things with a LOWER mass - selects the lower mass row as being i, the higher mass row as being j which is discarded if the conditions are met.
            potential_matches1= df[(df['m/z'] >= mz1) & (df['m/z'] < mz1 + 2) & 
            ...
            for j in potential_matches1.index:
                ...
                ccs2 = df.loc[j, 'CCS']
                intensity2 = df.loc[j, intensity_column]
                retention_time2 = df.loc[j, 'Retention Time']  # Get retention time of the second peak
            ...
            # Calculate ppm difference for a mass shift and percentage difference for CCS
                ...
                if diff_mass_shift < 2 and ccs_diff < 2 and intensity_diff > 50 and retention_time_diff < 0.5:
                    indices_to_keep.discard(j)
        ...
        # "M+1" Filter for CCS within 2% and mass within +1.0 to +1.01 units - eliminates M+1 peaks
            ...
            # Calculate ppm difference for a mass shift and percentage difference for CCS
                ...
                if 1.0 <= diff_mass_shift <= 1.01 and ccs_diff < 2 and retention_time_diff < 0.25:
                    indices_to_keep.discard(i)
    ...
    filtered_df = filtered_df[filtered_df.apply(lambda row: row['m/z'] * 0.19 + 110.28 > row['CCS'], axis=1)]

    # Separate tentative rows for EPA matching
    tentative_df = filtered_df[filtered_df['Score'].isin(['B+', 'B', 'B-', 'C+', 'C', 'C-', 'D-', 'D', 'D+'])].copy()
    ...
    def match_multiple_library_names(mz_value, library_masses, library_preferred_names, ppm_threshold=10):
        matches = []
        for mass in library_masses:
            ppm_diff = abs(mz_value - mass) / mass * 1e6
            if ppm_diff < ppm_threshold:
    ...
    tentative_df.loc[:, 'EPA_Match'] = tentative_df['m/z'].apply(
        lambda mz: match_multiple_library_names(mz, library_masses, library_preferred_names) is not None
    )

    # Update 'Name_or_Class' with all potential matches if they exist
    tentative_df.loc[:, 'Name_or_Class'] = tentative_df.apply(
        lambda row: match_multiple_library_names(row['m/z'], library_masses, library_preferred_names) or row['Name_or_Class'], axis=1
    )
